#![no_std]
use soroban_sdk::{
    contract, contractimpl, contracttype, symbol_short, Address, Env, String, Symbol, Vec,
};

// Struktur task
#[contracttype]
#[derive(Clone, Debug)]
pub struct Task {
    id: u64,
    title: String,
    description: String,
    completed: bool,
    owner: Address,
}

// Storage key
const TASK_DATA: Symbol = symbol_short!("TASK");

#[contract]
pub struct TodoContract;

#[contractimpl]
impl TodoContract {
    // GET TASKS
    pub fn get_tasks(env: Env) -> Vec<Task> {
        env.storage()
            .instance()
            .get(&TASK_DATA)
            .unwrap_or(Vec::new(&env))
    }

    // CREATE TASK
    pub fn create_task(env: Env, owner: Address, title: String, description: String) -> String {
        // 🔐 AUTH
        owner.require_auth();

        let mut tasks: Vec<Task> = env
            .storage()
            .instance()
            .get(&TASK_DATA)
            .unwrap_or(Vec::new(&env));

        let task = Task {
            id: env.prng().gen::<u64>(),
            title,
            description,
            completed: false,
            owner: owner.clone(),
        };

        tasks.push_back(task);

        env.storage().instance().set(&TASK_DATA, &tasks);

        String::from_str(&env, "Task created")
    }

    // UPDATE TASK
    pub fn update_task(env: Env, owner: Address, id: u64, title: String, description: String) -> String {
        owner.require_auth();

        let mut tasks: Vec<Task> = env
            .storage()
            .instance()
            .get(&TASK_DATA)
            .unwrap_or(Vec::new(&env));

        for i in 0..tasks.len() {
            let mut task = tasks.get(i).unwrap();

            if task.id == id {
                if task.owner != owner {
                    return String::from_str(&env, "Not owner");
                }

                task.title = title;
                task.description = description;

                tasks.set(i, task);
                env.storage().instance().set(&TASK_DATA, &tasks);

                return String::from_str(&env, "Task updated");
            }
        }

        String::from_str(&env, "Task not found")
    }

    // TOGGLE TASK
    pub fn toggle_task(env: Env, owner: Address, id: u64) -> String {
        owner.require_auth();

        let mut tasks: Vec<Task> = env
            .storage()
            .instance()
            .get(&TASK_DATA)
            .unwrap_or(Vec::new(&env));

        for i in 0..tasks.len() {
            let mut task = tasks.get(i).unwrap();

            if task.id == id {
                if task.owner != owner {
                    return String::from_str(&env, "Not owner");
                }

                task.completed = !task.completed;

                tasks.set(i, task);
                env.storage().instance().set(&TASK_DATA, &tasks);

                return String::from_str(&env, "Task toggled");
            }
        }

        String::from_str(&env, "Task not found")
    }

    // DELETE TASK
    pub fn delete_task(env: Env, owner: Address, id: u64) -> String {
        owner.require_auth();

        let mut tasks: Vec<Task> = env
            .storage()
            .instance()
            .get(&TASK_DATA)
            .unwrap_or(Vec::new(&env));

        for i in 0..tasks.len() {
            let task = tasks.get(i).unwrap();

            if task.id == id {
                if task.owner != owner {
                    return String::from_str(&env, "Not owner");
                }

                tasks.remove(i);
                env.storage().instance().set(&TASK_DATA, &tasks);

                return String::from_str(&env, "Task deleted");
            }
        }

        String::from_str(&env, "Task not found")
    }
}

mod test;













/* --- CONTOH SCRIPT ---

pub fn get_notes(env: Env) -> Vec<Note> {
    // 1. ambil data notes dari storage
    return env.storage().instance().get(&NOTE_DATA).unwrap_or(Vec::new(&env));
}

// Fungsi untuk membuat note baru
pub fn create_note(env: Env, title: String, content: String) -> String {
    // 1. ambil data notes dari storage
    let mut notes: Vec<Note> = env.storage().instance().get(&NOTE_DATA).unwrap_or(Vec::new(&env));
    
    // 2. Buat object note baru
    let note = Note {
        id: env.prng().gen::<u64>(),
        title: title,
        content: content,
    };
    
    // 3. tambahkan note baru ke notes lama
    notes.push_back(note);
    
    // 4. simpan notes ke storage
    env.storage().instance().set(&NOTE_DATA, &notes);
    
    return String::from_str(&env, "Notes berhasil ditambahkan");
}

// Fungsi untuk menghapus notes berdasarkan id
pub fn delete_note(env: Env, id: u64) -> String {
    // 1. ambil data notes dari storage 
    let mut notes: Vec<Note> = env.storage().instance().get(&NOTE_DATA).unwrap_or(Vec::new(&env));

    // 2. cari index note yang akan dihapus menggunakan perulangan
    for i in 0..notes.len() {
        if notes.get(i).unwrap().id == id {
            notes.remove(i);

            env.storage().instance().set(&NOTE_DATA, &notes);
            return String::from_str(&env, "Berhasil hapus notes");
        }
    }

    return String::from_str(&env, "Notes tidak ditemukan")
}


*/